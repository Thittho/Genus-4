...
Cubic1:=-x^2*y - x^2*z - 5*x^2*w - 6*x*y^2 - 2*x*y*z - 9*x*y*w + 7*x*z^2 + 3*x*z*w -
    ...
    10*z*w^2 + 3*w^3;
time Wgt, J1 := InvariantsGenus4Curves(Quadric1, Cubic1 : normalize := true);
...
time Wgt, J2 := InvariantsGenus4Curves(Quadric2, Cubic2 : normalize := true);

// Still some stuff to figure out here, since the invariants are not rationals but live in an extension...
// IsIsomorphic(Quadric1, Cubic1, Quadric2, Cubic2, Parent(J2[1]));
